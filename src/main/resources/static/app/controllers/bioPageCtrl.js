    ...
    .controller('bioPageCtrl', [
        ...
        plannerService.getPlannerByLink($routeParams.plannerLink)
            .then(function (data) {
                ...
                $rootScope.title = $location.path().replace('/event-host-page/','').replace( /\d+/,'').replace( /-/,'');
                $rootScope.pageDescription = $scope.plannerData.description;
                $rootScope.keyWord = $scope.plannerData.hostName;
